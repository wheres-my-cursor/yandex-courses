...
- стандартная библиотека Python, созданная для обработки коллекций.
- её вдохновляли функциональные языки, вроде APL и Haskell.
- с ее помощью можно строить итераторы, которые
    - экономят память
...
Чтобы функции библиотеки вызывались без префикса itertools
```
...
- Создает бесконечную числовую последовательность
- Можно задать старт и шаг
...
for value in count(0, 0.1):
    if value > 1:
        break
    print(round(value, 1))      # 2й аргумет - сколько знаком после запятой оставить
```

## cycle(): зацикливание коллекции
- Бесконечно перебирает элементы переданной коллекции
- Полезно, когда нужно "крутить" одно и то же: визуализация анимации, смена состояний или поочередное присвоение меток
...
- Позволяют объединить несколько итерируемых объектов в один, что быстрее и удобнее, чем склеивать списки вручную

```
from itertools import chain

values = list(chain("ABC", "DEF", "JHI"))
print(values)
...
values = list(chain.from_iterable(["ABC", "DEF", "JHI"]))
print(values)
...
- Создает все возможные упорядоченные комбинации элементов из нескольких коллекций
...
Аргумент repeat нужен, чтобы использовать каждую коллекцию два раза
```
from itertools import product
...
permutations(): перестановки без повторений
    Создает все возможные упорядоченные варианты из элементов.
    Через аргумент r можно указать другую длину, п/у - длина исходного объекта.
combinations(): сочетания без повторений
    Создает неупорядоченные комбинации длины r.
    Элементы внутри кортежей идут по порядку, но не повторяются.
combinations_with_replacement(): сочетания с повторениями

## enumerate()
Работает с одной коллекцией и добавляет к каждому элементу его индекс.
П/у индексация с нуля, но можно задать другое значение.

## zip()
Соединяет несколько коллекций, создавая кортежи из элементов с одинаковыми индексами.
